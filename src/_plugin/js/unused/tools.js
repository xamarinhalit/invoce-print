        ...
        $.fn.extend({
            ...
            Tools: function () {
                ...
                $('ul.m-drag-ul li', $ToolBox).draggable({
                    revert: 'invalid',
                    helper: "clone",
                    helper: function (ev, ui) {
                        const $elem = $(this);
                        const {left,top} = $elem.offset();
                        let dX = ev.pageX - left;
                        let dY = ev.pageY - top;
            ...
            createItem: function (page, ui) {
                ...
                let TableKey = $(this).data("tablekey");
                ...
                if (Type == "Table") {
                    ...
                    $('.Efar-Tool ul li[data-subkey="' + Key + '"]').css('display', 'block');
            ...
            propChange: function () {
                ...
                $('.Prop-Area input').on('keyup keypress focusout', function () {
                    ...
                    if ($('.m-Tool ul li.select').length == 0) {
                        ...
                        Key = selected.data('key');
                        Table = $('.efar-field[data-key="' + selected.data("subkey") + '"]')
                        FieldHeader = $('table.table .table-alignHeaders th[data-key="' + Key + '"]', Table);
                ...
                $('.FontFamily li').click(function () {
                    var selected = $('.Efar-Template-Page .active');
                    Key = selected.data('key');
            ...
            ruler: function () {
                ...
                RulerTop.html("");
                for (var i = 0; i < Math.ceil(PageWidthCm); i++) {
                    RulerTop.append("<li><span>" + i + "</span></li>");
                }
                RulerTop.append("<div class='clear'></div>");
                RulerLeft.html("");
                for (var i = 0; i < Math.ceil(PageHeightCm); i++) {
    ...
    export const getJsonData=()=> {
        ...
        var JsonTableData = [];
        var curCheck = "";
        var Page = $('.Efar-Template-Page-Area');
        $('.efar-field', Page).each(function () {
            ItemTitle = $(this).text();
            ...
            if ($(this).data("type") == "Table") {
                $('.Efar-Tool ul.sortable li[data-subkey="' + $(this).data("key") + '"]').each(function (index, item) {
                    JsonTableData.push({
                        TableKey: $(this).data("tablekey"),
                        ItemKey: $(this).data("key"),
